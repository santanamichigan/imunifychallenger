 ...
 *
 * Notes:
 * - Base path is this script's directory. Navigation is confined within it for safety.
 * - If you need a broader scope, place this file at the highest directory you own.
 */
...
function safe_join(string $base, string $path): string {
    ...
    // Absolute paths allowed only if inside base
    ...
    if ($real === false) {
        ...
        $real = rtrim($base, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . ltrim($path, DIRECTORY_SEPARATOR);
    }
    // Prevent breakout
...
function breadcrumb_html(string $base, string $path): string {
    ...
    $baseParts = explode(DIRECTORY_SEPARATOR, rtrim($baseReal, DIRECTORY_SEPARATOR));
    $parts = explode(DIRECTORY_SEPARATOR, trim(substr($real, strlen($baseReal)), DIRECTORY_SEPARATOR));
    $links = [];
    $accum = $baseReal;
    $links[] = '<a href="?p='.rawurlencode($baseReal).'" class="crumb"><i class="fa fa-home"></i> base</a>';
    $cur = $baseReal;
    foreach ($parts as $p) {
        if ($p === '' ) continue;
        $cur .= DIRECTORY_SEPARATOR . $p;
        $links[] = '<span class="sep">/</span><a class="crumb" href="?p='.rawurlencode($cur).'">'.htmlspecialchars($p).'</a>';
    }
